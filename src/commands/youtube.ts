...
export function registerYoutube(program: Command, getClient: () => Promise<CreatorCrawl>): void {
  ...
  youtube
    ...
    .action((handle: string) => run(async () => (await getClient()).youtube.channel({ handle })))
  ...
  youtube
    ...
    .action((url: string) => run(async () => (await getClient()).youtube.video({ url })))

  youtube
    .command('transcript <url>')
    .description('Get the transcript of a YouTube video')
    .action((url: string) => run(async () => (await getClient()).youtube.transcript({ url })))
  ...
  youtube
    ...
    .action(() => run(async () => (await getClient()).youtube.trendingShorts()))
}
